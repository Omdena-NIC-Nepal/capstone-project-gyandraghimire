...
def render():
    ...
    if task_type == "Classification":
        if target_task == "Glacier Retreat Severity (Multi-Class)":
            ...
            data = load_main()
            if target_task == "Heatwave Year (Binary)":
                data['heatwave_year'] = (data['heatwave_days'] >= 30).astype(int)
        ...
        st.subheader("⚙️ Train Random Forest Classifier")
        ...
        if st.button("🚀 Train Classifier"):
            ...
            joblib.dump(model, f"data/processed/interactive_model_{target_col}.joblib")
    ...
    # === REGRESSION ===
        ...
        st.subheader("⚙️ Train Random Forest Regressor")
        n_estimators = st.slider("Number of Trees", 50, 300, 100, 50)
        ...
        if st.button("🚀 Train Regressor"):
            ...
            st.bar_chart(pd.Series(model.feature_importances_, index=X.columns).sort_values(ascending=False).head(10))
            joblib.dump(model, f"data/processed/interactive_regressor_{target_col}.joblib")
